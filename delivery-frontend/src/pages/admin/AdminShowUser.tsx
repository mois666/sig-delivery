...
export const AdminShowUser = () => {
    const { id } = useParams();
    const navigate = useNavigate();
    const { user, wallet, showUser } = useUserStore();
    const { createWallet } = useWalletStore();
    const [loading, setLoading] = useState(true);

    const fetchUserDetail = async () => {
        try {
            await showUser(Number(id));
        } catch (error) {
    ...
    return (
        <div className="min-h-screen bg-background pb-24 safe-top">
            ...
            <div className="px-4 mt-6 space-y-6">
                ...
                >
                    <div className="w-24 h-24 rounded-3xl bg-primary/10 flex items-center justify-center mb-4 shadow-lg shadow-primary/20">
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full mt-6 border-t border-border/50 pt-6">
                        ...
                        <div className="flex flex-col items-center">
                            ...
                            <span className="font-bold">Nivel {user.level || 1}</span>
                ...
                {wallet ? (
                    ...
                    >
                        ...
                        <div className="mt-2">
                            ...
                            <h3 className="text-3xl font-display font-bold text-foreground">
                                Bs. {Number(wallet?.balance ?? user.wallet?.balance ?? 0).toFixed(2)}
                            </h3>
                        </div>

                        <div className="grid grid-cols-2 gap-2 mt-6">
                            <Button variant="secondary" className="w-full gap-2 text-xs rounded-xl">
                                <History className="w-3 h-3" /> Ver Movimientos
                            </Button>
                            <Button color="primary" className="w-full gap-2 text-xs rounded-xl">
                    ...
                    <div className="glass-card p-4 space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <Button color="primary" className="w-full gap-2 text-xs rounded-xl" onClick={handleCreateWallet}>
                ...
                <div className="glass-card p-4 space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[10px] text-muted-foreground uppercase font-bold">Correo Electrónico</p>
